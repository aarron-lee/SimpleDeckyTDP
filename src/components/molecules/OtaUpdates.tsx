...
const OtaUpdates = () => {
  ...
  return (
    <DeckySection title={t("SYSTEM_INFO_TITLE", "System Info")}>
      <ArrowToggleButton cacheKey="SystemInfoSectionCacheKey">
        <DeckyRow>
          <DeckyField
            label={t("SYSTEM_INFO_SYSTEM_LANGUAGE", "System Language")}
        ...
        <DeckyRow>
          <DeckyField
            label={t("SYSTEM_INFO_INSTALLED_VERSION", "Installed Version")}
            bottomSeparator="none"
          >
            {installedVersionNum}
          </DeckyField>
        ...
        {Boolean(latestVersionNum) && (
          <DeckyRow>
            <DeckyField
              label={t("SYSTEM_INFO_LATEST_VERSION", "Latest Version")}
              bottomSeparator="none"
            >
              {latestVersionNum}
            </DeckyField>
        ...
        {Boolean(scalingDriver) && (
          <DeckyRow>
            <DeckyField
              label={t("SYSTEM_INFO_SCALING_DRIVER", "Scaling Driver")}
              bottomSeparator="none"
            >
              {scalingDriver}
            </DeckyField>
        ...
        {Boolean(deviceName) && (
          <DeckyRow>
            <DeckyField
              label={t("SYSTEM_INFO_DEVICE_NAME", "Device Name")}
              bottomSeparator="none"
            >
              {deviceName}
            </DeckyField>
        ...
        <DeckyRow>
          <DeckyField
            label={t("SYSTEM_INFO_RESET_SETTINGS", "Reset Plugin Settings")}
            bottomSeparator="none"
          >
            {t(
              "SYSTEM_INFO_RESET_WARNING",
        ...
        {Boolean(latestVersionNum) && (
          <>
            <DeckyRow>
              <DeckyField
                label={t("SYSTEM_INFO_INFO", "Info")}
                bottomSeparator="none"
              >
                {isUpdated
                  ? t(
                      "SYSTEM_INFO_REINSTALL_TIME",
                      "Reinstall can take up to 1 minute",
                    )
                  : t(
                      "SYSTEM_INFO_UPDATE_TIME",
                      "Update can take up to 1 minute",
                    )}
              </DeckyField>
            </DeckyRow>
            <DeckyRow>
        ...
        {!isIntel && (
          <>
            <DeckyRow>
              <DeckyField
                label={t(
                  "SYSTEM_INFO_DETECT_AMD_UNDERVOLT",
                  "Detect AMD undervolt",
                )}
                bottomSeparator="none"
              >
                {t(
                  "SYSTEM_INFO_DETECT_AMD_UNDERVOLT_DESC",
